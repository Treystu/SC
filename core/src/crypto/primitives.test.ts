...
describe("Cryptographic Primitives", () => {
  ...
  describe("Session Key Management", () => {
    ...
    it("should rotate session keys", () => {
      ...
      expect(sessionKey2.timestamp).toBeGreaterThanOrEqual(
        sessionKey1.timestamp,
      );
    });
  });
  ...
  describe("ECDH Key Exchange", () => {
    it("should derive same shared secret for both parties", () => {
      ...
      if (!timingSafeEqual(aliceShared, bobShared)) {
        ...
        // eslint-disable-next-line no-console
        console.error(
          "[TEST-DEBUG] alice.privateKey=",
          toHex(alice.privateKey),
        );
        // eslint-disable-next-line no-console
    ...
    it("should validate key sizes", () => {
      ...
      expect(() => performKeyExchange(invalidPrivate, alice.publicKey)).toThrow(
        /32 bytes/,
      );
    });
  ...
  describe("Double Ratchet Algorithm", () => {
    ...
    it("should support forward secrecy through ratcheting", () => {
      ...
      // Keys should match
      expect(
        timingSafeEqual(aliceResult1.messageKey, bobResult1.messageKey),
      ).toBe(true);

      // Second message uses different key
      const aliceResult2 = deriveMessageKey(aliceState.sendChainKey);
      expect(
        timingSafeEqual(aliceResult1.messageKey, aliceResult2.messageKey),
      ).toBe(false);
    });
  });
  ...
  describe("Edge Cases and Security", () => {
    ...
    it("should reject tampering with ciphertext", () => {
      ...
      expect(() =>
        decryptMessage(ciphertext, sessionKey.key, sessionKey.nonce),
      ).toThrow();
    });
    ...
    it("should reject wrong key size for encryption", () => {
      ...
      expect(() =>
        encryptMessage(plaintext, wrongKey, sessionKey.nonce),
      ).toThrow(/32 bytes/);
    });
    ...
    it("should reject wrong nonce size for encryption", () => {
      ...
      expect(() =>
        encryptMessage(plaintext, sessionKey.key, wrongNonce),
      ).toThrow(/24 bytes/);
    });
    ...
    it("should reject ciphertext without auth tag", () => {
      ...
      expect(() =>
        decryptMessage(tooShort, sessionKey.key, sessionKey.nonce),
      ).toThrow(/too short/);
    });
  });
  ...
  describe("API Compatibility Aliases", () => {
    ...
    it("deriveSharedSecret should derive same secret as performKeyExchange with converted identity keys", () => {
      ...
      const xPriv = convertEd25519PrivateKeyToX25519(identity1.privateKey);
      const xPub = convertEd25519PublicKeyToX25519(identity2.publicKey);
      const secret2 = performKeyExchange(xPriv, xPub);
