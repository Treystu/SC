    ...
    if (recipientPublicKey && this.identity) {
      try {
        ...
        const sessionKey = deriveSessionKey(
          sharedSecret,
          new Uint8Array(0),
          new TextEncoder().encode("signaling"),
        );
      ...
      console.warn(
        `Sending unencrypted signal to ${to} because recipient public key is missing.`,
      );
    ...
    this.pollInterval = setInterval(async () => {
      try {
        ...
        if (this.lastMessageTimestamp > 0) {
          payload.payload = { since: this.lastMessageTimestamp };
        }
