...
//   global.hexToBytes =
//     require("./jest-mocks/@noble-hashes-utils.cjs").hexToBytes;
// }
...
// NOTE: Disabled because the shim uses fake crypto implementations
// Tests that need real crypto should import @noble libraries directly
try {
  if (
    false &&
    (!globalThis.crypto ||
...
// NOTE: Disabled for now due to verbose output
if (
  false &&
    ...
    try {
      ...
      for (const h of handles) {
        ...
        try {
          ...
          if (
            typeof h.remoteAddress !== "undefined" ||
            typeof h.remotePort !== "undefined"
          ) {
            entry.remoteAddress = h.remoteAddress;
            entry.remotePort = h.remotePort;
          }
          if (
            typeof h.localAddress !== "undefined" ||
            typeof h.localPort !== "undefined"
          ) {
      ...
      if (process.env.TEST_HANDLE_STACKS) {
        try {
          ...
          // eslint-disable-next-line no-console
          console.log(
            "[TEST-DEBUG] Running why-is-node-running to get handle stacks...",
          );
          ...
          // eslint-disable-next-line no-console
          console.error(
            "[TEST-DEBUG] why-is-node-running failed:",
            e && e.stack ? e.stack : e,
          );
        }
      }
