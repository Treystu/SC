...
test.describe("P2P Messaging", () => {
  test("should send message between two peers", async ({ browser }) => {
    ...
    // Wait for peers to be connected on both sides
    await Promise.all([
      page1.waitForFunction(() => (window as any).peers?.length > 0),
      page2.waitForFunction(() => (window as any).peers?.length > 0),
    ]);
    ...
    // Use the exposed sendMessage function if available, or UI
