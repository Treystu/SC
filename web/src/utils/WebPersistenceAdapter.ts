...
export class WebPersistenceAdapter implements PersistenceAdapter {
  ...
  async getMessage(_id: string): Promise<StoredMessage | null> {
  ...
  async fetchUpdates(_docName: string): Promise<Uint8Array[]> {
    return [];
  }
  ...
  async pruneExpired(_now: number): Promise<void> {
    // No-op
  }
