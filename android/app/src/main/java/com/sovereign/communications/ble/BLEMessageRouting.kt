...
class BLEMessageRouting {
    ...
    var onSendDirect: ((String, ByteArray) -> Boolean)? = null
    ...
    data class RouteEntry(
        ...
        fun isExpired(currentTime: Long): Boolean = (currentTime - lastUpdated) > ROUTE_EXPIRY_MS
    ...
    data class RouteDiscoveryRequest(
        ...
        fun isExpired(currentTime: Long): Boolean = (currentTime - requestTime) > timeout
    }
    ...
    fun updateRouteMetrics(
        ...
        val newLatency =
            if (route.avgLatencyMs > 0) {
                (route.avgLatencyMs * 0.7 + latencyMs * 0.3).toLong()
            } else {
                latencyMs
            }

        routingTable[destinationId] =
            route.copy(
            ...
            scope.launch {
                while (isActive) {
                    ...
                    if (request != null) {
                        ...
                        onBroadcast?.invoke(packet) ?: Log.w(TAG, "Transport layer not initialized, cannot broadcast")
                    }
                }
            }
    }
        ...
        return mapOf(
            ...
            "averageHopCount" to
                if (routes.isNotEmpty()) {
                    routes.map { it.hopCount }.average()
                } else {
                    0.0
                },
            "averageReliability" to
                if (routes.isNotEmpty()) {
                    routes.map { it.reliability }.average()
                } else {
                    0.0
                },
            "averageQuality" to
                if (routes.isNotEmpty()) {
                    routes.map { it.getQualityScore() }.average()
                } else {
                    0.0
                },
            "hopDistribution" to
