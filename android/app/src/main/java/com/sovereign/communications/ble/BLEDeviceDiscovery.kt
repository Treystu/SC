...
class BLEDeviceDiscovery(private val context: Context) {
    ...
    data class DiscoveredDevice(
        ...
        fun getAverageRssi(): Int {
            return if (rssiHistory.isNotEmpty()) {
        ...
        fun isExpired(currentTime: Long): Boolean {
            return (currentTime - lastSeen) > DEVICE_TIMEOUT_MS
        }
     ...
     */
    private fun updateDiscoveredDevice(existing: DiscoveredDevice, result: ScanResult): DiscoveredDevice {
     ...
     */
    fun getDevicesByDistance(category: DistanceCategory): List<DiscoveredDevice> {
        return getDevices(DiscoveryFilter(distanceCategory = category))
    }
    ...
    fun getClosestDevice(filter: DiscoveryFilter = DiscoveryFilter()): DiscoveredDevice? {
        return getDevices(filter).maxByOrNull { it.getAverageRssi() }
    }
    ...
    fun getRankedDevices(
        ...
        return getDevices(filter).take(limit)
    }
    ...
    fun getStats(): Map<String, Any> {
        ...
        return mapOf(
            ...
            "byDistanceCategory" to devices.groupBy { it.getDistanceCategory() }
                .mapValues { it.value.size },
            "averageRssi" to if (devices.isNotEmpty()) 
                devices.map { it.getAverageRssi() }.average() else 0.0,
            "averageScans" to if (devices.isNotEmpty())
                devices.map { it.scanCount }.average() else 0.0,
            "topDevice" to getClosestDevice()?.let {
                mapOf(
