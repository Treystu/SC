...
class BLEReassembly(
    ...
    private data class ReassemblyBuffer(
        ...
        fun isExpired(
            currentTime: Long,
            timeout: Long,
        ): Boolean = currentTime - timestamp > timeout
    ...
    fun addFragment(header: BLEFragmentation.FragmentHeader): ReassemblyResult {
        ...
        try {
            val buffer =
                buffers.getOrPut(header.messageId) {
                    ReassemblyBuffer(
                        header.messageId,
                        header.total,
                        header.crc32,
    ...
    private fun cleanupExpired() {
        ...
        val expiredKeys = buffers.filterValues { it.isExpired(currentTime, timeoutMs) }.keys
     ...
     */
    fun getFailedMessages(): List<FailureRecord> = failedMessages.values.toList()
